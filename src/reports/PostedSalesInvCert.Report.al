report 50023 "Posted Sales Inv Cert."
{
    DefaultLayout = RDLC;
    RDLCLayout = 'src/reportlayout/PostedSalesInvCert.rdl';
    Caption = 'Sales - Invoice Certificate';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(AmtInclVAT_SalesInvLine; AmountToCustomer())
                        {
                            AutoFormatType = 1;
                        }
                        ...
                        column(TotalGSTAmount_SalesInvoiceLine; SGSTAmt + CGSTAmt + IGSTAmt)//"Sales Invoice Line"."Total GST Amount"
                        {
                        }
            ...
            begin
                ...
                SalesInvoiceLine3.SETFILTER(SalesInvoiceLine3."Document No.", "Sales Invoice Header"."No.");
                SalesInvoiceLine3.SETRANGE(SalesInvoiceLine3.Type, SalesInvoiceLine3.Type::" ");
                IF SalesInvoiceLine3.FINDSET() THEN
                    REPEAT

                        IF (STRLEN(SalesInvoiceLine3.Description) >= 13) THEN   // ZE_Kajal 15-6-2020  added this line
                            ShipTxt := COPYSTR(SalesInvoiceLine3.Description, 13, (STRLEN(SalesInvoiceLine3.Description) - 13))
                        ELSE
                            ShipTxt := COPYSTR(SalesInvoiceLine3.Description, 13); //ZE_Kajal 15-6-2020  added this line

                        IF SalesShipmentHeader1.GET(ShipTxt) THEN BEGIN
                            SalesOrder.RESET;
                            SalesOrder.SETFILTER(SalesOrder."No.", SalesShipmentHeader1."Order No.");
                            IF SalesOrder.FINDFIRST THEN
                                IF (STRLEN(Desc3) + STRLEN(SalesShipmentHeader1."External Document No.")) < 1024 THEN
                                    Desc3 += SalesShipmentHeader1."External Document No." + ' & ';


                        END;
                    UNTIL SalesInvoiceLine3.NEXT = 0;
                IF (STRLEN(SalesInvoiceLine3.Description) > 5) THEN
                    Desc3 := COPYSTR(Desc3, 1, (STRLEN(Desc3) - 3));
                //--
